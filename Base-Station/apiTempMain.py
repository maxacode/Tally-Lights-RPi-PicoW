...
app = Microdot()
clients = {}
current_button_state = None  # Initialize global variable for button state
...
def send_button_update(state):
    if state is not None:
        ...
        print("Sending changes to clients")
        for ip in clients.keys():
            try:
                ...
                response = requests.post(url,headers=headers,timeout = 1.5)
...
async def create_client(request):
    # ... (rest of the logic for creating a client remains the same)
